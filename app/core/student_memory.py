...
def build_last_session_summary(question: str, answer: str, mode: str) -> str:
    if mode != "quiz_mc":
        return answer

    try:
        data = json.loads(answer)
        topic = data.get("topic", "Unbekannt")
        questions = data.get("questions", [])
        return f"Quiz zu {topic} erzeugt. {len(questions)} Fragen."
    except Exception:
        return "Quiz erzeugt."
...
def update_progress_from_question(
    ...
    progress["last_session_summary"] = build_last_session_summary(question, answer, mode)
